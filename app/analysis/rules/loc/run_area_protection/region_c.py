...
class BoundLocRunAreaProtectionRegionCRule(BoundObstacleRule):
    ...
    def analyze(self, obstacle: dict[str, object]) -> AnalysisRuleResult:
        ...
        return AnalysisRuleResult(
            ...
            message=(
                "obstacle type not restricted by run area critical standard"
                if not is_applicable
                else (
                    "obstacle outside run area critical region"
                    if not entered_protection_zone
                    else "obstacle enters run area critical region"
                )
            ),
            metrics={
                "areaType": "critical",
...
class LocRunAreaProtectionRegionCRule(ObstacleRule):
    rule_code = "loc_run_area_protection_region_c"
    rule_name = "loc_run_area_protection_region_c"
    zone_code = "loc_run_area_protection"
    zone_name = "LOC run area protection zone"
    ...
    def bind(
        ...
        *,
        ...
        region_geometry = build_loc_run_area_region_c_geometry(shared_context)
        return BoundLocRunAreaProtectionRegionCRule(
