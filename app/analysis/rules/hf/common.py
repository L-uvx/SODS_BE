...
from app.analysis.rules.hf.config import HF_STANDARDS_RULE_CODE_BY_CATEGORY
...
class BoundHfCircleRule(BoundObstacleRule):
    station_point: tuple[float, float]
    minimum_distance_meters: float

    # 按障碍物分类解析 HF 条文键，避免共用规则档位时标准错配。
    def _resolve_standards_rule_code(self, obstacle: dict[str, object]) -> str:
        category = str(obstacle["globalObstacleCategory"])
        return HF_STANDARDS_RULE_CODE_BY_CATEGORY.get(category, self.protection_zone.rule_code)
    ...
    def analyze(self, obstacle: dict[str, object]) -> AnalysisRuleResult:
        ...
        return AnalysisRuleResult(
            ...
            message=(
                "obstacle outside hf protection zone"
                if is_compliant
                else "obstacle entered hf protection zone"
            ),
            metrics={
...
def build_hf_circle_protection_zone(
    *,
    station: object,
    rule_code: str,
    rule_name: str,
